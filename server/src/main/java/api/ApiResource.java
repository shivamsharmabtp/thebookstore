...
public class ApiResource {
    ...
    public List<Book> suggestedBooks(@PathParam("category-id") long categoryId,
        ...
        try {
            ...
            throw new ApiException("suggestedBook lookup via categoryId failed", e);
        }
    }
    ...
    public  Category categoryByName(@PathParam("category-name") String categoryName,
        ...
        try {
            ...
            if(result == null){
                throw new ApiException(String.format("No such category found for categoryName: %s", categoryName));
            }
            return result;
        } catch (Exception e) {
            throw new ApiException("category lookup by category-name failed", e);
        }
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public  List<Book> booksByCategoryByName(@PathParam("category-name") String categoryName,
                                             @Context HttpServletRequest request){
        try {
            Category category = categoryDao.findByName(categoryName);
            if(category == null){
                throw new ApiException(String.format("No books found as no such category found for categoryName: %s", categoryName));
            }
            long categoryId = category.getCategoryId();
            List<Book> books = bookDao.findByCategoryId(categoryId);
            return books;
        } catch (Exception e) {
            throw new ApiException("books lookup by category-name failed", e);
        }
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public  List<Book> suggestedBooksByCategoryByName(@PathParam("category-name") String categoryName,
                                                      @QueryParam("limit") @DefaultValue("3") int limit,
                                                      @Context HttpServletRequest request){
        try {
            Category category = categoryDao.findByName(categoryName);
            ...
            long categoryId = category.getCategoryId();
            List<Book> books = bookDao.findRandomByCategoryId(categoryId, limit);
            return books;
        } catch (Exception e) {
            throw new ApiException("books lookup by category-name failed", e);
        }
